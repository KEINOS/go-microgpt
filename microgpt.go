...
func rmsnorm(x []*Value) []*Value {
	...
	scale := pow(add(ms, newValue(1e-5, nil, nil)), -0.5)
...
func initStateDict(vocabSize int) StateDict {
	...
	for i := range nLayer {
		...
		// MLP weights
		sd[key+".mlp_fc1"] = matrix(4*nEmbd, nEmbd, initStd) // expand layer
		sd[key+".mlp_fc2"] = matrix(nEmbd, 4*nEmbd, initStd) // contract layer
	}
...
func train(numSteps int, docs []string, uchars []rune, BOS, vocabSize int, stateDict StateDict, optimizer *adamOptimizer) {
	for step := range numSteps {
		...
		allTokens := make([]int, 0, len(tokens)+2)
...
// ============================================================================
// Section 13.5: Inference & Sampling
// ============================================================================

// sample generates a single sample from the model.
func sample(temperature float64, maxLen int, uchars []rune, BOS, vocabSize int, stateDict StateDict) string {
	keys := make([][][]*Value, nLayer)
